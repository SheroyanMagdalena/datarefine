...
export default function NewProject() {
  ...
  const handleSave = async () => {
    ...
    else if (dataFormat === "json") fileType = "csv";//toDo: which type should be allowed
    ...
    try {
      ...
      try {
        const updateRes = await fetch("/api/user/project", {
          method: "POST",
          headers: {
        ...
        if (!updateRes.ok) {
          ...
          console.error("Failed to update user projectName:", err || updateRes.statusText);
        } else {
          const updatedUser = await updateRes.json();
          console.log("✅ User updated with projectName:", updatedUser);
        }
      } catch (e) {
        console.error("Error calling /api/user/project:", e);
      }
